...
public class QndTestConsumeManual {
    ...
    private static void queueAndConsume(KafkaClient kafkaClient, String topic, String consumerGroupId, int numItems)
            throws InterruptedException {

        System.out.print("Putting [" + numItems + "] items to queue [" + topic + " / " + consumerGroupId + "]...");
        ...
        int counter = 0;
        KafkaMessage kmsg = kafkaClient.consumeMessage(consumerGroupId, false, topic, 1000, TimeUnit.MILLISECONDS);
        while (kmsg != null) {
            counter++;
            kmsg = kafkaClient.consumeMessage(consumerGroupId, false, topic, 1000, TimeUnit.MILLISECONDS);
        }
        System.out.println(
                "Consumed [" + counter + "] items from queue [" + topic + " / " + consumerGroupId + "] in " + (
    ...
    public static void main(String[] args) throws Exception {
        ...
        String topic = System.getProperty("topic", "t1partition");
